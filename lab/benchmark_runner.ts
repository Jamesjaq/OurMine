 ...
 *  12. Scorecard Generation (lab/results/capability_report.json & .md)
 */
...
export async function runBenchmark() {
  ...
  // Read ground truth
  const groundTruthRaw = fs.readFileSync("lab/ground_truth/manifest.json", "utf8")
  ...
  const gobEv = graph.makeEvidence("gobuster", `gobuster dir -u http://${groundTruth.target}:8080/ -w lab/wordlist.txt`, gobusterRaw, 2500)
...
`
  ...
  log(`\n[REPORT] Saved capability reports to lab/results/capability_report.json and capability_report.md`)
}
