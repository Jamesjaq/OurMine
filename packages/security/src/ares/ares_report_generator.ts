 ...
 * Automatically generates high-impact PDF mission reports using Typst.
 */
...
export async function generateMissionReportPdf(config: ReportConfig, opts: { live?: boolean } = {}) {
  ...
  const findingsRows = config.findings.map((f, idx) => `
+ *${f.title}* (#raw("${f.id}")):
  - *Severity:* *${f.severity}*
...
`
  ...
  try {
    ...
    throw new Error(`[OurMine] PDF Report compilation failed: ${err.message}`)
  }

  const findings = config.findings.map(f => realFinding(f.id, f.title, f.severity as any, f.description))
  return moduleEnvelope(live, { pdfPath, typPath, status: "PDF_REPORT_GENERATED" }, findings)
}
