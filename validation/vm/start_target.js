...
export function startTargetServer() {
  const targetScript = path.resolve("validation/vm/target_server.js")
  const outLog = fs.openSync(path.resolve("validation/vm/target.log"), "a")
  ...
  child.unref()
  fs.writeFileSync(path.resolve("validation/vm/.target.pid"), String(child.pid))
  return child.pid ?? 0
}
